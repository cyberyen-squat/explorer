...
from helpers.helpers import bulk_of_first_run_or_cron, JSONRPC, pre_boogie
...
def lets_boogie(the_blocks, uniques, cryptocurrency):
    ...
    for block_height in the_blocks:
        try:
            ...
            cronjob.logger.error(f"Block {block_height} added. Waiting for new blocks.")
            db.session.rollback()
            db.session.close()
            sys.exit()
...
if __name__ == '__main__':
    ...
    if most_recent_stored_block != most_recent_block:
        ...
        lets_boogie(all_the_blocks, chain_params, rpc_call)
    else:
        sys.exit()
