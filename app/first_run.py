...
from helpers.helpers import bulk_of_first_run_or_cron, JSONRPC, JSONRPCException, pre_boogie
from utils.chain import Cyberyen
from utils.models import db, Blocks, MWEBBlocks

EXPECTED_TABLES = {'addresses', 'address_summary', 'blocks', 'mweb_blocks', 'coinbasetxin', 'txs', 'txout', 'txin'}
    ...
    with click.progressbar(the_blocks, item_show_func=process_block) as progress_bar:
        for block_height in progress_bar:
            ...
            # If disk is full we can't log anything.. so, shutdown.
                ...
                first_run_app.logger.error(f"Block {block_height} added. Waiting for new blocks.")
                db.session.rollback()
                db.session.close()
                sys.exit()
...
def detect_chain(cryptocurrency):
    try:
...
if __name__ == '__main__':
    ...
    try:
        ...
        uniques = detect_chain(rpc_call)

        if autodetect_tables:
            detect_tables()

        most_recent_block = rpc_call.getblockcount()
        ...
        try:
            ...
            lets_boogie(all_the_blocks, rpc_call, 0)
            ...
            if most_recent_stored_block != most_recent_block:
                ...
                start_time = time.strftime('%Y/%m/%d - %H:%M:%S')
                lets_boogie(all_the_blocks, rpc_call, block_length)
            else:
