...
from utils.models import Blocks, MWEBBlocks, TXs, TxOut, TXIn, Addresses, AddressSummary, CoinbaseTXIn
...
def bulk_of_first_run_or_cron(name_of_flask_app, db, params, cryptocurrency, block_height, total_blocks,
    ...
    try:
        ...
        for number, this_transaction in enumerate(raw_block_transactions):
            ...
            if block_height >= 2880 and number == len(raw_block_transactions) - 1:
                mweb_transactions.append(this_transaction)
                continue
        ...
        if block_height == 0:
            prev_block_hash = params['genesis']['prev_hash']
        ...
        if block_height >= 2880 and mweb_transactions:
            for txid in mweb_transactions:
                mweb_block_info = the_block['mweb']
                mweb_block_entry = MWEBBlocks(
                    height=mweb_block_info['height'],
                    hash=mweb_block_info['hash'],
                    kernel_offset=mweb_block_info['kernel_offset'],
                    stealth_offset=mweb_block_info['stealth_offset'],
                    num_kernels=mweb_block_info['num_kernels'],
                    num_txos=mweb_block_info['num_txos'],
                    kernel_root=mweb_block_info['kernel_root'],
                    output_root=mweb_block_info['output_root'],
                    leaf_root=mweb_block_info['leaf_root'],
                )
                db.session.add(mweb_block_entry)
